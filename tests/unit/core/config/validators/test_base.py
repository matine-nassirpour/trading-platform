...
def test_validation_result_success_ok_type():
    result = ValidationResult(ok=True, message=None, value="ok", rule="core.rule")
    value = result.raise_if_failed()

    assert result.ok is True
    assert value == "ok"
    assert result.rule == "core.rule"


def test_validation_result_failure_raises():
    result = ValidationResult(ok=False, message="error", value=None, rule="core.fail")
